...
module _ {X : Set} where
  ...
  _+th_ : forall {n m p q} -> n <= m -> p <= q -> n +B p <= m +B q
  th +th (p -, x) = th +th p -, x
  th +th (p -^ x) = th +th p -^ x
  th +th [] = th
  ...
  thinrLemma : forall {n m}(th : n <= m) p ->
    (thinr {_}{p} ^^ (th +th iota {p})) ~ thinr
  ...
  data All (P : X -> Set) : Bwd X -> Set where
    [] : All P []
    _-,_ : forall {xz x} -> All P xz -> P x -> All P (xz -, x)

  pb : forall {p n m}(th : p <= m)(ph : n <= m) ->
       _ >< \ q ->
       q <= p >< \ th' -> q <= n >< \ ph' ->
       (th' ^^ th) ~ (ph' ^^ ph)
  pb (th -^ x) (ph -^ .x) with pb th ph
  ... | _ , th' , ph' , q = _ , th' , ph' , ((_-^ x) $~ q)
  pb (th -^ x) (ph -, .x) with pb th ph
  ... | _ , th' , ph' , q = _ , th' , ph' -^ x , ((_-^ x) $~ q) 
  pb (th -, x) (ph -^ .x) with pb th ph
  ... | _ , th' , ph' , q = _ , th' -^ x , ph' , ((_-^ x) $~ q) 
  pb (th -, x) (ph -, .x) with pb th ph
  ... | _ , th' , ph' , q = _ , th' -, x , ph' -, x , ((_-, x) $~ q)
  pb [] [] = [] , [] , [] , r~

  pbCo : forall {p n m}(th : p <= n)(ph : n <= m) ->
    pb ph (th ^^ ph) ~ (p , th , iota , sym (iota^^ _))
  pbCo th (ph -^ x) with pb ph (th ^^ ph) | pbCo th ph
  ... | _ | r~ = (_ ,_) $~ ((_ ,_) $~ ((_ ,_) $~ uip))
  pbCo (th -^ .x) (ph -, x) with pb ph (th ^^ ph) | pbCo th ph
  ... | _ | r~ = (_ ,_) $~ ((th -^ x ,_) $~ ((iota ,_) $~ uip))
  pbCo (th -, .x) (ph -, x) with pb ph (th ^^ ph) | pbCo th ph
  ... | _ | r~ = (_ ,_) $~ ((th -, x ,_) $~ ((iota ,_) $~ uip))
  pbCo [] [] = r~
